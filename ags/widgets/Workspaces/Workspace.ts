            ...
            setup: (eventbox) => {
                ...
                eventbox.connect(
                    ...
                    (_w, _c, _x, _y, data) => {
                        ...
                        dispatch(
                            `movetoworkspacesilent ${id},address:${address}`
                        );
                    }
                );
            },
            child: Widget.Fixed().hook(
                hyprland,
                (fixed) => {
                    ...
                    hyprland.clients
                        ...
                        .forEach((c) => {
                            const x =
                                c.at[0] -
                                (hyprland.getMonitor(c.monitor)?.x || 0);
                            const y =
                                c.at[1] -
                                (hyprland.getMonitor(c.monitor)?.y || 0);
                            c.mapped &&
                                fixed.put(Window(c), scale(x), scale(y));
                        });
                    fixed.show_all();
                },
                "notify::clients"
